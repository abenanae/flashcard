...
最新版ではアプリ本体はネットワーク優先で取得します。古いService Workerが残っている端末では一度だけ再読み込み、またはDevToolsでService WorkerだけをUnregisterしてください。
ブラウザのサイトデータ削除はIndexedDB内のカードや学習履歴も消すため、Google Drive同期が済むまで実行しないでください。

Chromeで `読み込み中です。` のまま止まる場合は、IndexedDBのバージョン更新が別タブやホーム画面PWAにブロックされています。
同じアプリを開いているChromeタブ、別ウィンドウ、PWAをすべて閉じてから再読み込みしてください。

## データ保存

アプリのバージョン更新やService Workerキャッシュ更新では、IndexedDB内のカードや学習履歴は削除されません。
ただし、ブラウザごと、通常/シークレットごと、iPhone PWAごとにIndexedDBは別です。
同じデータを複数端末・複数ブラウザで使うにはGoogle Drive同期を使ってください。
